...
- **3레인 진군 바닥선** — `battlefield_depth_lane` 3개를 배경에 남겨 성, 아군, 적 스폰/진군선이 같은 지면 축으로 읽히게 했다.
- **교전 중 맥락 유지** — 교전 phase 진입 후 `IsoBaseLayer`는 계속 숨기지만 바닥 밴드와 레인은 유지해 유닛이 빈 배경 위에서 떠오르는 느낌을 줄인다.
- **검증** — `test_unit_walk_visuals.gd`가 persistent band/lane 구조를 검증하고, `tools/ui_feedback_smoke.gd`가 배경 지면 밴드, 3레인, 교전 중 유닛 y 범위를 확인한다. `./init.sh` 카드 22개, 단위 테스트 3043/3043 green.

## 2026-06-06 — feat-076 장기런 전투 템포 계약
중후반 전투가 25~35초 이상 늘어져 관전 피로가 생기는 문제를 자동 검증으로 잡고, 보스 encounter 체력을 좁게 조정했다.
- **시간 예산 helper** — `LongRunTempoContract`가 일반/정예/중간 보스 24초, 최종 보스 28초, 군주별 평균 18초 예산을 제공한다.
- **장기런 실패 조건** — `tools/long_run_smoke.gd`가 각 교전 시간과 군주별 평균 시간을 검사해 예산 초과 시 실패한다.
- **보스 encounter 조정** — `WaveFactory._boss_encounter()`의 동탁/장각/여포 encounter HP를 낮춰 보스 존재감은 유지하되 장기런이 늘어지지 않게 했다.
- **검증** — 유비/조조/손권 장기런이 평균 13.6/15.9/10.5초로 통과한다. `./init.sh` 카드 22개, 단위 테스트 3040/3040 green.

## 2026-06-06 — feat-075 전장 보드 지면화와 교전 가시성 보정
배치 보드가 하늘에 뜬 9칸 UI처럼 보이고, 교전 시작 때 유닛이 필드 뒤에서 나타나는 느낌을 줄이도록 전장 투영과 격자 전환을 보정했다.
- **전장 투영 하향** — `battle.gd`의 view origin을 아래로 내리고 y scale을 더 압축해 보드 중심이 화면 지면 밴드에 놓이게 했다.
- **타일 접지 보강** — tile diamond 반폭/반높을 실제 256×128 타일 텍스처의 0.75 scale과 맞추고, 각 타일 아래 `battlefield_tile_contact` shadow를 추가했다.
- **교전 가시성** — 교전 phase 진입 직후 `IsoBaseLayer`를 숨겨 전투 유닛과 배치 격자가 겹쳐 보이는 시간을 제거했다.
- **검증** — `test_unit_walk_visuals.gd`가 보드 지면 밴드와 타일 간격 계약을 검증하고, `tools/ui_feedback_smoke.gd`가 지면 밴드, 접지 shadow, 교전 중 격자 숨김을 확인한다. `./init.sh` 카드 22개, 단위 테스트 3018/3018 green.

## 2026-06-06 — feat-074 초반 군세 밀도 계약 강화
첫 전투와 초반 루프가 빈약한 1대1 전투처럼 보이지 않도록 병종/장수 시작 군세와 자동 metric 기준을 강화했다.
- **분대 시작 규모 상향** — `SquadProfile.base_squad_count()`가 일반 병종 12명, 기병 10명, 수군 11명, fantasy 8명으로 시작한다. 레벨업은 3명씩 증가하며 기존 18명 렌더 cap에 닿는다.
- **장수 호위 강화** — 장수 Lv.1 호위를 7명으로 올려 장수 카드가 단독 대형 스프라이트가 아니라 지휘 축과 호위병 묶음으로 보이게 했다.
- **초반 metric 계약 강화** — `PlaytestMetrics.first_five_ok()`가 매 교전 아군 12명 이상, 전체 30명 이상, 피크 아군 30명 이상, 22초 max/19초 avg를 검증한다.
- **검증** — playtest loop smoke stage 1/2/5가 아군 12/20/32명, 전체 37/36/42명으로 통과한다. `./init.sh` 카드 22개, 단위 테스트 3012/3012 green.
