...
static func boss_waves() -> Array:
	var spawn_x := BattleSim.FIELD_W
	return [[
		_enemy_unit(1, spawn_x, "마왕 동탁", 2300, 48, 0.9, "melee", 28.0, "infantry", "highest_hp"),
...
static func _enemy_unit(lane: int, x: float, display_name: String, hp: int, attack: int, interval: float, attack_range: String, speed: float, troop_type: String, target_rule: String = "nearest") -> BattleUnit:
	return BattleUnit.make(BattleUnit.Team.ENEMY, lane, x, display_name, hp, attack, interval, attack_range, speed, &"", &"", troop_type, -1, BattleSim.start_y_for_col(lane), false, target_rule)
