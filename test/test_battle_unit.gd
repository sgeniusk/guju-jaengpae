# BattleUnit의 순수 런타임 상태와 카드 변환을 검증한다.
extends TestCase
...
func test_make_maps_runtime_stats() -> void:
	var unit := BattleUnit.make(BattleUnit.Team.ENEMY, 1, 900.0, "사령병", 90, 14, 1.2, "melee", 34.0, &"enemy_dead")
	eq(unit.team, BattleUnit.Team.ENEMY, "팀 매핑")
	eq(unit.lane, 1, "레인 매핑")
	almost(unit.x, 900.0, 0.001, "위치 매핑")
