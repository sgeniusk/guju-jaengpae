...
## 현재 상태
**마지막 갱신** — 2026-06-07
**활성 피처** — feat-087 읽히는 바닥 칸 seam 완료
**현재 목표** — 완성판까지 Codex goal을 유지한다. 이번 단위는 접지감을 유지하려고 낮춘 보드 표식이 너무 약해 바닥 칸이 안 보이는 문제를 바닥 seam으로 보강한 작업이다.

## 완료
- [x] **feat-087 읽히는 바닥 칸 seam** — `battle.gd`가 각 배치 타일에 어두운 홈과 얇은 밝은 lip으로 된 `TileFloorSeam` 2개를 추가했다. 기존 tile fill과 `battlefield_tile_outline` alpha는 낮게 유지해 공중판 회귀를 막고, seam alpha만 0.11 이상 0.18 이하로 잡아 바닥 칸 경계가 읽히게 했다. `test_unit_walk_visuals.gd`와 UI smoke가 seam count와 alpha 범위를 검증한다. `./init.sh` 카드 22개 / 3150 단언 green.
- [x] **feat-086 배치 ghost와 분대 개별 동세** — `battle.gd`에 `DeployPreviewLayer`를 추가해 손패 유닛 선택 후 빈 타일 hover 시 실제 formation body를 반투명 ghost로 보여준다. ghost는 `CardCatalog.build_board_army()`와 `_create_unit_body()` 경로를 재사용하지만 RunState/BattleSim에는 넣지 않는다. formation member마다 `formation_home`/`formation_phase`/`formation_index` 메타를 저장하고 전투 중 보폭, 긴장 흔들림, 공격 lunge를 서로 다르게 적용해 한 덩어리 이동감을 줄였다. `test_unit_walk_visuals.gd`와 UI smoke가 ghost 생성/접지/제거, 분대 구성원 개별 motion을 검증한다. `./init.sh` 카드 22개 / 3147 단언 green.
...
- [ ] 완성판 안전 개선 계속 — 다음 후보는 배치 카드 UI 정리, GUI 표시 드라이버 screenshot bundle 재확인, 실제 플레이에서 바닥 seam/ghost/분대 동세 재확인이다.
...
## 이번 세션 수정 파일
- `docs/specs/feat-087-readable-floor-cells.md`
- `docs/specs/feat-086-deploy-ghost-and-squad-motion.md`
...
## 검증 증거
- [x] `HOME=$PWD/.godot/home godot --headless --path . --log-file .godot/feat-087-unit.log --script res://test/runner.gd` (2026-06-07, feat-087) — 단위 테스트 3150/3150 green.
- [x] `HOME=$PWD/.godot/home godot --headless --path . --log-file .godot/feat-087-ui.log --script res://tools/ui_feedback_smoke.gd` (2026-06-07, feat-087) — 바닥 seam count/alpha 포함 UI smoke green.
- [x] `./init.sh` (2026-06-07, feat-087) — 카드 22개 검증 OK, UI smoke, 저장/이어하기 smoke, playtest loop, 장기런 tempo gate, 단위 테스트 3150/3150 포함 전체 green.
- [x] `HOME=$PWD/.godot/home godot --headless --path . --log-file .godot/feat-086-unit.log --script res://test/runner.gd` (2026-06-07, feat-086) — 단위 테스트 3147/3147 green.
- [x] `HOME=$PWD/.godot/home godot --headless --path . --log-file .godot/feat-086-ui.log --script res://tools/ui_feedback_smoke.gd` (2026-06-07, feat-086) — 배치 hover ghost와 분대 개별 motion 포함 UI smoke green.
- [x] `./init.sh` (2026-06-07, feat-086) — 카드 22개 검증 OK, UI smoke, 저장/이어하기 smoke, playtest loop, 장기런 tempo gate, 단위 테스트 3147/3147 포함 전체 green.
...
## 다음 세션 메모
feat-087 done. 다음 안전 피처는 배치 카드 UI surface 정리 또는 GUI 표시 드라이버 screenshot bundle 재확인이 좋다. 실제 사용자가 보는 GUI에서 바닥 seam, 배치 ghost, 분대 개별 동세는 다시 플레이 확인이 필요하다. 천계·마계 확장은 정본 승인 전 시작하지 않는다. push와 tag는 사용자 확인 후에만 실행한다.
