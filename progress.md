...
**현재 목표 (Current Objective)** — feat-015 경제·보드 상태 모델 1단계 구현 완료. 다음은 시각 플레이 QA와 feat-015b 보드 기반 전투/UI, feat-015c 상점·드래프트. 로드맵 — docs/roadmap.md.
...
2. feat-015b 보드 기반 전투 + 배치/손패 UI — 전투 스폰 소스를 영속 보드로 전환.
3. feat-015c 상점 노드 + 보상 드래프트 + 우물 UI — 골드 소비와 손패 초과 해소.
4. feat-016 건물(building) 카드 + 오라 — NK 핵심 요소.
...
- feat-015 — scripts/run/run_state.gd, scripts/autoloads/run_manager.gd, scripts/run/reward_pool.gd
- 테스트/스모크 — test/test_run_board.gd 신설, test/test_run_reward.gd, test/test_map_nodes.gd, tools/reward_smoke.gd
...
## 다음 세션 메모
`./init.sh`로 베이스라인(541단언) 확인. RunState는 시작 덱을 3×3 영속 보드에 배치하고, RunManager.get_deck()은 보드 카드만 반환한다. 보상 add_card()는 빈 보드 블록 우선, 가득 차면 손패로 간다. 다음 큰 덩어리는 feat-015b 전투 스폰/UI 전환이며, 단위 테스트는 `res://test/runner.gd`가 `test/test_*.gd` 수집.
