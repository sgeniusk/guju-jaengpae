...
- [x] **feat-033/G045~G053 저장 포맷+payload+profile+result+resume+unlock+boundary** (Codex) — `PersistenceStore`, primitive payload, `save_version`, ProfileState API, 전투 결과 기록/해금/새 런 overlay, 런 autosave/이어하기, 신규/미래 버전 테스트, unlock-aware 보상/군주 선택, 프로필 저장/로드와 저장 I/O 경계를 구현했다. **feat-034/G054/G057/G063 + feat-035/G064~G070 + feat-036/G071~G077 + feat-037/G078~G082** — 9세력 게이트, act·보스 구조, 결과 화면, UI 툴팁/피드백, walk/배경/오디오/온보딩/HUD placeholder 감소, UI 스크린샷 묶음, Phase 7 밸런스 수치 계약, macOS export preset/pack export, 릴리스 문서·태그 체크리스트, fresh clone green을 닫았다. ./init.sh 2349 단언 green.
### 진행 중 (What's In Progress)
- [ ] Phase 7 full app export 준비. Phase 4/G055·G056 마계·천계 데이터 확장은 사용자 명칭 승인 전 blocked.
### 다음 (What's Next)
1. 사용자 승인으로 천계·마계 명칭이 canon이 되면 G055/G056을 재개한다.
2. 승인 전에는 full app export와 리스크 문서화처럼 nation id 확정이 필요 없는 작업을 먼저 고른다.
...
- [ ] full app export는 G083에서 Godot export templates와 실행 증거가 필요하다.
...
- [x] `./init.sh` (2026-06-04, feat-037/G079~G082) → 카드 **22개** 검증 OK, Phase 5/6/7 스모크와 밸런스/export preset 계약, 단위 테스트 **2349 단언** green. pack export 성공, 산출물 33MB는 `build/` ignore. 로컬 fresh clone에서도 `./init.sh` green. 종료 0.
...
## 다음 세션 메모
`./init.sh` 2349단언 green. **feat-031 G026~G033, feat-032 G034~G044, feat-033 G045~G053, feat-034 G054/G057/G063, feat-035 G064~G070, feat-036 G071~G077, feat-037 G078~G082 done** — Phase 4 9세력 확장은 정본 승인→CardVocab→validator→Resource→lord_select 순서가 고정됐고 G055/G056은 명칭 승인 대기 blocked. 다음은 full app export 준비. 푸시는 사용자 확인 후.
