...
## 현재 상태 (2026-06-05) — feat-041 전투 체감 패스 완료
- `./init.sh` 카드 **22개 / 2645 단언 green**. Godot 4.6.3 기준 군주 3 검증, run_map/lord_select/battle/보스 stage/결과 화면/UI 피드백/첫 5스테이지 플레이테스트 루프 스모크가 통과했다. feat-038에서 런 전투를 성 위치 선택→손패 3장 중 1장 배치 또는 계략 사용→즉시 단일 교전 루프로 바꿨고, feat-039에서 유닛 카드를 분대/증원 성장 단위로 바꿨으며, feat-040에서 이 루프를 MVP 제품 계약과 순수 helper/자동 메트릭으로 고정했다. feat-041은 첫 전투 적 전열과 시작 돌격 피드백을 추가했다.
...
- **현재 local history** — 현재 작업 브랜치는 `codex/feat-040-mvp`다. feat-040 MVP 기준점은 `5a10dda feat: lock fun reset MVP`로 커밋했다. push/tag는 사용자 확인 전 금지다.
...
2. **완성판 안전 작업** — 수동 플레이 감각 QA, 장기런 자동 스모크, 전투 중 표적 지정 체감, 카드 선택/보상 전략성 강화처럼 nation id 확정 없이 가능한 작업을 feat-042 이후 하나씩 고른다.
...
## Codex 시작 프롬프트 (복사)
> 구주쟁패(/Users/taewookkim/dev/guju-jaengpae) 이어서. 현재 브랜치는 `codex/feat-040-mvp`. feat-038~040으로 성 선점→3장 손패→1장 플레이→분대/증원 성장 MVP 루프를 만들고, feat-041로 첫 전투 적 전열과 "전군 돌격!" 시작 VFX를 추가했다. 최신 `./init.sh` 카드 22개 / **2645 green**. 플레이테스트 루프는 stage 1/2/5 전투 21.1s/18.3s/14.6s, stage 1 enemy visible 25명·total 35명으로 통과한다. G055/G056/G058/G060/G061/G062는 명칭 승인 대기 blocked이므로 사용자/편집장 승인 전 천계·마계 nation id와 Resource를 추가하지 않는다. push/tag는 사용자 확인 전 금지. 다음은 feat-042로 장기런/수동 플레이 QA 자동화, 카드 선택 전략성, 전투 중 표적 지정 체감 중 하나를 잡아 `docs/specs/` 스펙 → 구현 → `./init.sh` green → 상태 파일 갱신 → 중요 커밋 순서로 진행한다.
