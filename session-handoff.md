...
- **현재 local history** — 현재 작업 브랜치는 `codex/feat-040-mvp`다. feat-040 MVP 기준점은 `5a10dda feat: lock fun reset MVP`, feat-041은 `46a882a feat: improve first battle feel`, feat-042는 `dd923f4 feat: add formation tactics`, feat-043은 `8706667 feat: preview formation bonuses`로 커밋했다. feat-044는 이번 세션 커밋 대상으로 정리한다. push/tag는 사용자 확인 전 금지다.
...
2. **완성판 안전 작업** — 수동 플레이 감각 QA, 전투 중 표적 지정 체감, 카드 선택/보상 전략성 강화, 장기런 군주 3종 확대처럼 nation id 확정 없이 가능한 작업을 feat-045 이후 하나씩 고른다.
...
## Codex 시작 프롬프트 (복사)
> 구주쟁패(/Users/taewookkim/dev/guju-jaengpae) 이어서. 현재 브랜치는 `codex/feat-040-mvp`. feat-038~040으로 성 선점→3장 손패→1장 플레이→분대/증원 성장 MVP 루프를 만들고, feat-041로 첫 전투 적 전열과 "전군 돌격!" 시작 VFX를 추가했으며, feat-042로 지휘/엄호/측면 진형 전술 보너스와 타일 태그를, feat-043으로 배치 전술 미리보기를, feat-044로 최종 보스까지 장기런 스모크를, feat-045로 전투 중 집중표적 피드백을 추가했다. 최신 `./init.sh` 카드 22개 / **2701 green**. 플레이테스트 루프는 stage 1/2/5 전투 21.1s/18.3s/14.6s, 장기런은 stage 15 result=1·20.5s·wins=8으로 통과한다. G055/G056/G058/G060/G061/G062는 명칭 승인 대기 blocked이므로 사용자/편집장 승인 전 천계·마계 nation id와 Resource를 추가하지 않는다. push/tag는 사용자 확인 전 금지. 다음은 feat-046으로 수동 플레이 QA 자동화, 카드 선택/보상 전략성, 장기런 군주 3종 확대 중 하나를 잡아 `docs/specs/` 스펙 → 구현 → `./init.sh` green → 상태 파일 갱신 → 중요 커밋 순서로 진행한다.
